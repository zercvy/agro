...
const PlotForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Загружаем типы почвы из базы данных
    const fetchSoilTypes = async () => {
      try {
        const res = await API.get('/soil-types');
        setSoilTypes(res.data); // Устанавливаем типы почвы
      } catch (err) {
        console.error('Ошибка при загрузке типов почвы:', err);
      }
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const payload = {
      name,
      type,
      soil_type: soilType,  // Используем id типа почвы
      coordinates: geometry,
      area,
      perimeter,
    };
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-4 bg-white shadow rounded p-6">
        ...
        {/* Тип почвы загружается из базы данных */}
        <select
          className="w-full border p-2 rounded"
          value={soilType}
          onChange={(e) => setSoilType(e.target.value)}
